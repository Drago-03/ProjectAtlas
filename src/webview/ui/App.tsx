...
interface InitState {
  version?: string;
  directory?: {
    nodes: { id: string; name: string; type: string; parent?: string }[];
  };
  workflows?: { nodes: any[]; edges: any[] };
  symbols?: any;
}
...
export const App: React.FC = () => {
      ...
      switch (msg?.type) {
        ...
        case 'MARKDOWN_RENDER': {
          ...
          const found = Array.from(
            msg.payload.html.matchAll(
              /<code class=\"language-mermaid\">([\s\S]*?)<\/code>/g
            )
          ).map((m: any) => decodeHtml(m[1]));
          if (found.length) setPendingMermaid(found);
          break;
        }
        case 'GRAPH_UPDATE':
          setInit((i) => (i ? { ...i, symbols: msg.payload } : i));
          break;
        case 'MERMAID_RESULT':
          setMermaids((list) => [...list, msg.payload]);
          break;
        case 'MERMAID_ERROR':
          setMermaidErrors((errs) => [
            ...errs,
            msg.payload?.error || 'Unknown mermaid error',
          ]);
          break;
        case 'SYMBOL_PATCH':
          setInit((i) =>
            i ? { ...i, symbols: patchSymbols(i.symbols, msg.payload) } : i
          );
          break;
        case 'WORKFLOW_UPDATE':
          setInit((i) => (i ? { ...i, workflows: msg.payload } : i));
          break;
      }
    };
  ...
  function toggleTheme() {
    ...
    setMermaids([]); // re-render
    // re-request mermaid diagrams for new theme
    const codes = Array.from(
      markdown.matchAll(/<code class=\"language-mermaid\">([\s\S]*?)<\/code>/g)
    ).map((m: any) => decodeHtml(m[1]));
  ...
  return (
    <div className="container">
      <h1>
        ProjectAtlas <span className="badge">{init?.version || '…'}</span>{' '}
        ...
        <div className="panel scroll">
          ...
          {mermaids.length > 0 && (
            <div>
              ...
              <button
                className="btn-small"
                onClick={() => exportDiagrams(mermaids)}
              >
                Export SVGs
              </button>
              {mermaids.map((m) => (
                <div key={m.id} dangerouslySetInnerHTML={{ __html: m.svg }} />
              ))}
            </div>
          )}
          {mermaidErrors.length > 0 && (
            <div className="error-block">
              <h3>Diagram Errors</h3>
              <ul>
